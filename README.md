...
sanskar = {
    ...
    "stack"       : ["Java", "Spring Boot", "Python", "Flask", "TypeScript", "PostgreSQL"],
    "strengths"   : ["System Design", "API Architecture", "Testing & CI/CD", "Cryptography"],
...
> My background in applied security means I think about what happens when things go wrong — not just when they go right.
...
`TypeScript` `AES-256-GCM` `Argon2id` `Browser Extension` `React`

A zero-knowledge password manager where the server never sees plaintext. Vault encrypted with AES-256-GCM, keys derived via Argon2id KDF, with a phishing-aware browser extension and Android/iOS system autofill support.

```
Encryption    : AES-256-GCM vault encryption
KDF           : Argon2id (memory-hard, side-channel resistant)
Extension     : Phishing-aware browser autofill
Mobile        : Android/iOS system autofill integration
Server Model  : Stores ciphertext only — zero knowledge
```

Designed around the principle that trust should never be required — only cryptographic proof.
...
### Clarifact — AI Fact-Checking Tool
`JavaScript` `Node.js` `AI/NLP`

An AI-powered fact-checking and claim verification tool — built to surface credible sources and flag misinformation in real time.

```
Core          : AI/NLP-powered claim analysis
Architecture  : Lightweight Node.js backend
Status        : Active development
```

Built at the intersection of natural language understanding and information integrity.

[![View Repo](https://img.shields.io/badge/View_Repo-181717?style=flat-square&logo=github&logoColor=white)](https://github.com/Sanskar-bot/clarifact)
...
### Cybersecurity Portfolio — Personal Site
`React` `TypeScript` `Vite`

A modern cybersecurity portfolio with a living learning timeline — built with React and TypeScript, featuring a custom knowledge system that automatically tracks and visualizes daily progress across all projects and research.

```
Stack         : React + TypeScript + Vite
Feature       : Auto-tracked daily learning timeline
...
A structured daily log of what I study and build — started April 2026, committed every day without gaps.
...
Format        : Structured markdown — challenges + root cause
Streak        : April 2026 → present (no gaps)
```

Not a highlight reel. A real engineering journal.

[![View Log](https://img.shields.io/badge/View_Log-181717?style=flat-square&logo=github&logoColor=white)](https://github.com/Sanskar-bot/Daily-Learnings)
...
echo "Production mindset   — I don't just write code. I write code that survives."
```
